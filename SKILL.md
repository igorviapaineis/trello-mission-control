...
User --chat--> Orchestrator
              ...
              Trello board
                  ...
                  v
              Executor (owns this column — card never leaves it)
                  |
                  ...
                  v
   card stays in the column; archive_old.py sweeps done-labelled cards on a timer
   [optional pipeline mode] next <id> --expect <my-list> hands off to the next column
```
...
- `claim-<me>` is the doing / em-andamento signal. Finishing a card calls `done <card_id> <me>` which adds the `done` label, sets the native `dueComplete` check, and releases the claim — **the card never leaves your column** (status is a label, not a list). `reopen <card_id>` undoes it.
- After work (or aborting without finishing), call `release <card_id> <agent>` to free the claim.
